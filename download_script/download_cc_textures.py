...
def cli():
    ...
    while True:
        ...
        json_url = f"https://ambientcg.com/api/v2/full_json?include=downloadData&limit={offset_size}" \
                   f"&offset={current_offset}&type=material,atlas"
    ...
    including_list = ["wood", "leafset", "leaf"]
    excluding_list = ["woodfloor", "woodsiding", "woodchlp"]
    ...
    for index, (asset, content) in enumerate(data.items()):
        ...
        do_use = False
        for include_element in including_list:
