...
import time
import bmesh
from src.blender.object_pool import load_ccmaterials
...
class Tree(object):
    ...
    __slots__ = (
        ...
        'branch_mat', 'leaf_mats', 'blossom_mats'
    )

    def __init__(self, param, material_dir, generate_leaves=True):
        ...
        branch_mats = load_ccmaterials(folder_path=material_dir, used_assets=['wood'], load_cnt=1)
        self.branch_mat = random.choice(branch_mats)

        random_hue = random.random()

        LEAF_MAT_FOR_ONE_TREE = 4
        self.leaf_mats = load_ccmaterials(folder_path=material_dir, used_assets=['leaf', 'leafset'],
                                          load_cnt=LEAF_MAT_FOR_ONE_TREE, given_hue=random_hue)

        BLOSSOM_MAT_FOR_ONE_TREE = 4
        self.blossom_mats = load_ccmaterials(folder_path=material_dir, used_assets=['leaf', 'leafset'],
                                             load_cnt=BLOSSOM_MAT_FOR_ONE_TREE, given_hue=random_hue)
    ...
    def make(self):
        ...
        self.tree_obj = bpy.data.objects.new('Tree', tree_mesh)
    ...
    def create_branches(self):
        ...
        for level_depth, level_name in enumerate(['Trunk'] + ['Branches' + str(_) for _ in range(1, self.param.levels)]):
            ...
            level_obj.active_material = self.branch_mat.blender_obj
    ...
    def create_leaf_mesh(self):
        ...
        if leaf_index > 0:
            ...
            for mat in self.leaf_mats:
                leaves_obj.data.materials.append(mat.blender_obj)
            bpy.ops.object.select_all(action='DESELECT')
            leaves_obj.select_set(True)
            bpy.ops.object.mode_set(mode='EDIT')
            bm = bmesh.from_edit_mesh(leaves_obj.data)
            no_of_materials = len(leaves_obj.material_slots)
            for face in bm.faces:
                idx = random.randint(0, no_of_materials - 1)
                face.material_index = idx
            leaves_obj.data.update()  # Update the mesh from the bmesh data
        ...
        if blossom_index > 0:
            ...
            for mat in self.blossom_mats:
                blossom_obj.data.materials.append(mat.blender_obj)
            bpy.ops.object.select_all(action='DESELECT')
            blossom_obj.select_set(True)
            bpy.ops.object.mode_set(mode='EDIT')
            bm = bmesh.from_edit_mesh(blossom_obj.data)
            no_of_materials = len(blossom_obj.material_slots)
            for face in bm.faces:
                idx = random.randint(0, no_of_materials - 1)
                face.material_index = idx
            blossom_obj.data.update()  # Update the mesh from the bmesh data
            bpy.ops.object.mode_set(mode='OBJECT')

            blossom.uv_layers.new(name="blossomUV")
...
def construct(params, material_dir, generate_leaves=True):
    """Construct the tree"""

    t = Tree(TreeParam(params), material_dir, generate_leaves)
    t.make()
